...
class zCrawler:
    ...
    def getHotels(self):
        ...
        self.cur.execute(
            'SELECT p.id as package_id, p.name_en, p.name_cn, pm.city, pm.country  FROM package as p LEFT JOIN package_meta as pm ON p.id = pm.package_id ' + sqlWhere)
    ...
    def savePrice(self, priceHotel):
        ...
        for k, v in priceHotel.iteritems():
            if (k in ['package_id', 'name_en', 'name_cn', 'target_url', 'target_site', 'lowest_price', 'query_date',
    ...
    def getSearchUrlCtrip(self, detail):
        ...
        if (domestic == 0):
            return self.rootUrl['ctrip'] + 'international/' + str(
                locationId) + '/k2' + hotelName + '?checkin=' + self.checkIn + '&checkout=' + self.checkOut
        else:
            return self.rootUrl['ctrip'] + 'hotel/' + str(
                locationId) + '/k1' + hotelName + '?checkin=' + self.checkIn + '&checkout=' + self.checkOut
    ...
    def getSearchUrlQunar(self, detail):
        ...
        hotelName = str(detail['name_en'])
        return self.rootUrl['qunar'] + 'city/' + cityPath + '/q-' + hotelName + '#fromDate=' + str(
            detail['check_in_date']) + '&toDate=' + str(detail['check_out_date'])
    ...
    def getZanaduDetailUrl(self, detail):
        ...
        return self.rootUrl['zanadu'] + 'hotel/selectRooms?package_id=' + str(
            detail['package_id']) + '&check_in=' + str(detail['check_in_date']) + '&check_out=' + str(
    ...
    def queryCtrip(self, detail):
        ...
        if (lowestPrice == 'NF'):
            ...
            countNights = datetime.datetime.strptime(self.checkOut, '%Y-%m-%d') - datetime.datetime.strptime(
                self.checkIn, '%Y-%m-%d')
    ...
    def exportToCsv(self):
        ...
        csvWhereSql = ' WHERE `query_date` = "' + str(self.queryDate) + '" AND `check_in_date`="' + str(
            self.checkIn) + '" AND `check_out_date`="' + str(self.checkOut) + '"'
